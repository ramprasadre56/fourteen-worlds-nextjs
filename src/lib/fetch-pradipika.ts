...
// Cache the result for 1 hour to avoid excessive requests
let cachedData: PradipikaIssue[] | null = null;
let cacheTime: number = 0;
const CACHE_DURATION = 60 * 60 * 1000; // 1 hour

export async function fetchBhagavataPradipikaIssues(): Promise<PradipikaIssue[]> {
    ...
    try {
        ...
        links.forEach((link) => {
            ...
            if (href && text && href.endsWith('.pdf')) {
                ...
                // Pattern: "Bhagavata Pradipika Issue XX-Title - YYYY-MM"
                ...
                if (issueMatch) {
                    ...
                    // Extract title - everything between issue number and date
                    let title = text
...
function getFallbackIssues(): PradipikaIssue[] {
    return [

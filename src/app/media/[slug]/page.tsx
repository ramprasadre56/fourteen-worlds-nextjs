'use client';

import { use } from 'react';
import Link from 'next/link';
import { PlayCircle, ExternalLink, BookOpen, ChevronRight, Share2, Check } from 'lucide-react';
import { getMediaPlaylistBySlug, ALL_MEDIA_PLAYLISTS, MEDIA_INSTRUCTOR, type MediaPlaylist } from '@/data/media-data';
import { useLearning } from '@/contexts/LearningContext';
import { notFound } from 'next/navigation';

/* ─── Progress Bar ──────────────────────────────────────── */
function ProgressBar({ percent, height = 6 }: { percent: number; height?: number }) {
    return (
        <div className="w-full rounded-full overflow-hidden" style={{ background: 'rgba(139, 26, 26, 0.1)', height }}>
            <div className="h-full rounded-full transition-all duration-500" style={{
                width: `${percent}%`,
                background: percent === 100
                    ? 'linear-gradient(90deg, #5c7a1f, #7ca82f)'
                    : 'linear-gradient(90deg, var(--color-primary), var(--color-primary-light))',
            }} />
        </div>
    );
}

/* ─── Related Card ──────────────────────────────────────── */
function RelatedCard({ playlist }: { playlist: MediaPlaylist }) {
    const { isEnrolled, getProgressPercent } = useLearning();
    const enrolled = isEnrolled(playlist.slug);
    const percent = getProgressPercent(playlist.slug);

    return (
        <Link
            href={`/media/${playlist.slug}`}
            className="group flex items-center gap-3 p-3 rounded-xl cursor-pointer"
            style={{
                background: 'var(--color-bg-warm)',
                border: '1px solid var(--color-border-light)',
                transition: 'all var(--transition-base)',
            }}
            onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--color-secondary)'; }}
            onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--color-border-light)'; }}
        >
            <span className="text-xl flex-shrink-0">{playlist.icon}</span>
            <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate" style={{ color: 'var(--color-text)' }}>
                    {playlist.title}
                </p>
                <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
                    {playlist.videoCount} episodes
                    {enrolled && ` • ${percent}%`}
                </p>
            </div>
            <ChevronRight size={14} style={{ color: 'var(--color-text-muted)' }} className="group-hover:translate-x-1 transition-transform flex-shrink-0" />
        </Link>
    );
}

/* ─── Main Page ─────────────────────────────────────────── */
export default function MediaDetailPage({ params }: { params: Promise<{ slug: string }> }) {
    const { slug } = use(params);
    const playlist = getMediaPlaylistBySlug(slug);
    const {
        isEnrolled, enrollInCourse, getProgressPercent, getCompletedCount,
        markLessonComplete, markLessonIncomplete, getProgress,
    } = useLearning();

    if (!playlist) notFound();

    const enrolled = isEnrolled(playlist.slug);
    const percent = getProgressPercent(playlist.slug);
    const completed = getCompletedCount(playlist.slug);
    const progress = getProgress(playlist.slug);

    const related = ALL_MEDIA_PLAYLISTS
        .filter(p => p.category === playlist.category && p.slug !== playlist.slug)
        .slice(0, 4);
    const relatedFinal = related.length > 0 ? related : ALL_MEDIA_PLAYLISTS.filter(p => p.slug !== playlist.slug).slice(0, 4);

    const categoryLabel = playlist.category === 'bg' ? 'Bhagavad Gītā'
        : playlist.category === 'sb' ? 'Śrīmad Bhāgavatam'
        : playlist.category === 'vedic-stories' ? 'Vedic Epics' : 'Topical Series';

    return (
        <div className="min-h-screen" style={{ background: 'var(--color-bg-warm)' }}>
            {/* Hero */}
            <section className="relative overflow-hidden py-10 md:py-14" style={{
                background: playlist.category === 'bg'
                    ? 'linear-gradient(135deg, rgba(139, 26, 26, 0.95) 0%, rgba(107, 16, 16, 0.97) 100%)'
                    : playlist.category === 'sb'
                        ? 'linear-gradient(135deg, rgba(184, 146, 46, 0.9) 0%, rgba(139, 26, 26, 0.95) 100%)'
                        : playlist.category === 'vedic-stories'
                            ? 'linear-gradient(135deg, rgba(107, 66, 38, 0.95) 0%, rgba(75, 45, 25, 0.97) 100%)'
                            : 'linear-gradient(135deg, rgba(92, 122, 31, 0.9) 0%, rgba(75, 45, 25, 0.95) 100%)',
            }}>
                <div className="absolute inset-0 opacity-10" style={{
                    backgroundImage: 'radial-gradient(circle at 80% 30%, rgba(212, 168, 83, 0.3), transparent 50%)',
                }} />
                <div className="relative max-w-[1200px] mx-auto px-8">
                    {/* Breadcrumb */}
                    <div className="flex items-center gap-2 text-xs mb-5 flex-wrap" style={{ color: 'rgba(245, 237, 224, 0.5)' }}>
                        <Link href="/media" className="hover:text-white transition-colors cursor-pointer">Media</Link>
                        <ChevronRight size={10} />
                        <span style={{ color: 'rgba(245, 237, 224, 0.8)' }}>{playlist.title}</span>
                    </div>

                    <div className="flex items-start gap-4 mb-3">
                        <span className="text-4xl md:text-5xl flex-shrink-0">{playlist.icon}</span>
                        <div>
                            <h1 className="text-2xl md:text-4xl font-bold mb-1" style={{
                                fontFamily: 'var(--font-heading)', color: '#F5EDE0', lineHeight: 1.2,
                            }}>
                                {playlist.title}
                            </h1>
                            <p className="text-sm" style={{ color: 'rgba(212, 168, 83, 0.8)', fontFamily: 'var(--font-heading)' }}>
                                {playlist.subtitle}
                            </p>
                        </div>
                    </div>

                    <p className="text-base max-w-3xl mb-4" style={{ color: 'rgba(245, 237, 224, 0.7)', lineHeight: 1.6 }}>
                        {playlist.description}
                    </p>

                    {/* Meta + Enroll */}
                    <div className="flex items-center gap-4 flex-wrap">
                        <span className="flex items-center gap-1 text-sm" style={{ color: 'rgba(245, 237, 224, 0.6)' }}>
                            <PlayCircle size={14} /> {playlist.videoCount} Episodes
                        </span>
                        <span className="flex items-center gap-1 text-sm" style={{ color: 'rgba(245, 237, 224, 0.6)' }}>
                            <BookOpen size={14} /> {playlist.instructor}
                        </span>

                        {enrolled ? (
                            <span className="ml-auto flex items-center gap-2 text-sm font-semibold px-4 py-1.5 rounded-full" style={{
                                background: 'rgba(92, 122, 31, 0.2)', color: '#a8d35f',
                            }}>
                                <Check size={14} /> Enrolled • {percent}% complete
                            </span>
                        ) : (
                            <button
                                onClick={() => enrollInCourse(playlist.slug, playlist.videoCount)}
                                className="ml-auto text-sm font-bold px-6 py-2.5 rounded-lg cursor-pointer"
                                style={{
                                    background: 'linear-gradient(135deg, #ffffff 0%, #F5EDE0 100%)',
                                    color: 'var(--color-primary)',
                                    border: 'none',
                                    boxShadow: '0 4px 16px rgba(0,0,0,0.2)',
                                }}
                            >
                                Enroll Now — Free
                            </button>
                        )}
                    </div>

                    {enrolled && (
                        <div className="mt-4">
                            <ProgressBar percent={percent} height={4} />
                        </div>
                    )}
                </div>
            </section>

            {/* Content */}
            <div className="max-w-[1200px] mx-auto px-8 py-8">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    {/* Main */}
                    <div className="lg:col-span-2">
                        {/* YouTube Embed */}
                        <div className="card-elevated overflow-hidden mb-6">
                            <div className="aspect-video w-full">
                                <iframe
                                    src={`https://www.youtube.com/embed/videoseries?list=${playlist.youtubePlaylistId}`}
                                    className="w-full h-full"
                                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
                                    allowFullScreen
                                    title={playlist.title}
                                    style={{ border: 'none' }}
                                />
                            </div>
                        </div>

                        {/* Lesson Tracker */}
                        {enrolled && (
                            <div className="card-warm p-5 mb-6">
                                <div className="flex items-center justify-between mb-4">
                                    <h2 className="text-lg font-bold" style={{
                                        fontFamily: 'var(--font-heading)', color: 'var(--color-primary)',
                                    }}>
                                        Lesson Progress
                                    </h2>
                                    <span className="text-xs font-semibold" style={{ color: 'var(--color-text-muted)' }}>
                                        {completed}/{playlist.videoCount} completed
                                    </span>
                                </div>

                                <div className="mb-4">
                                    <ProgressBar percent={percent} height={8} />
                                </div>

                                <div className="grid grid-cols-5 sm:grid-cols-8 md:grid-cols-10 gap-2">
                                    {Array.from({ length: playlist.videoCount }, (_, i) => {
                                        const isCompleted = progress?.lessons[i]?.completed || false;
                                        return (
                                            <button
                                                key={i}
                                                onClick={() => isCompleted ? markLessonIncomplete(playlist.slug, i) : markLessonComplete(playlist.slug, i)}
                                                className="w-full aspect-square rounded-lg flex items-center justify-center cursor-pointer text-xs font-bold"
                                                title={`Lesson ${i + 1} — ${isCompleted ? 'Completed' : 'Not started'}`}
                                                style={{
                                                    background: isCompleted
                                                        ? 'linear-gradient(135deg, #5c7a1f, #7ca82f)'
                                                        : 'var(--color-bg)',
                                                    color: isCompleted ? 'white' : 'var(--color-text-muted)',
                                                    border: isCompleted ? 'none' : '1px solid var(--color-border)',
                                                    transition: 'all var(--transition-fast)',
                                                }}
                                            >
                                                {isCompleted ? <Check size={12} /> : i + 1}
                                            </button>
                                        );
                                    })}
                                </div>
                                <p className="text-xs mt-3 text-center" style={{ color: 'var(--color-text-muted)' }}>
                                    Click a lesson number to mark it complete/incomplete
                                </p>
                            </div>
                        )}

                        {/* About + CTAs */}
                        <div className="card-warm p-6 mb-6">
                            <h2 className="text-lg font-bold mb-3" style={{
                                fontFamily: 'var(--font-heading)', color: 'var(--color-primary)',
                            }}>About This Series</h2>
                            <p className="text-sm mb-4" style={{ color: 'var(--color-text-secondary)', lineHeight: 1.7 }}>
                                {playlist.description}
                            </p>
                            <div className="flex flex-wrap gap-2 mb-4">
                                {playlist.tags.map((tag) => (
                                    <span key={tag} className="text-xs px-2.5 py-1 rounded-full" style={{
                                        background: 'var(--color-bg-deep)', color: 'var(--color-text-secondary)',
                                        border: '1px solid var(--color-border)',
                                    }}>{tag}</span>
                                ))}
                            </div>
                            <div className="flex flex-wrap gap-3 pt-3" style={{ borderTop: '1px solid var(--color-border-light)' }}>
                                <a href={playlist.youtubePlaylistUrl} target="_blank" rel="noopener noreferrer"
                                    className="btn-primary inline-flex items-center gap-2">
                                    <ExternalLink size={14} /> Open in YouTube
                                </a>
                                <button onClick={() => navigator.clipboard.writeText(playlist.youtubePlaylistUrl)}
                                    className="btn-golden inline-flex items-center gap-2 cursor-pointer">
                                    <Share2 size={14} /> Share
                                </button>
                            </div>
                        </div>
                    </div>

                    {/* Sidebar */}
                    <div className="lg:col-span-1">
                        <div className="card-elevated p-5 mb-6">
                            <h3 className="text-base font-bold mb-4" style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-primary)' }}>
                                Series Details
                            </h3>
                            <div className="flex flex-col gap-3">
                                {[
                                    ['Episodes', `${playlist.videoCount}`],
                                    ['Category', categoryLabel],
                                    ['Speaker', playlist.instructor],
                                    ['Price', 'FREE'],
                                ].map(([label, value]) => (
                                    <div key={label}>
                                        <div className="flex items-center justify-between">
                                            <span className="text-xs" style={{ color: 'var(--color-text-muted)' }}>{label}</span>
                                            <span className="text-sm font-bold" style={{
                                                color: label === 'Price' ? '#5c7a1f' : 'var(--color-text)',
                                            }}>{value}</span>
                                        </div>
                                        <div className="h-px mt-3" style={{ background: 'var(--color-border-light)' }} />
                                    </div>
                                ))}
                                {enrolled && (
                                    <div>
                                        <div className="flex items-center justify-between">
                                            <span className="text-xs" style={{ color: 'var(--color-text-muted)' }}>Progress</span>
                                            <span className="text-sm font-bold" style={{ color: 'var(--color-primary)' }}>{percent}%</span>
                                        </div>
                                        <div className="mt-2">
                                            <ProgressBar percent={percent} height={4} />
                                        </div>
                                    </div>
                                )}
                            </div>

                            {enrolled ? (
                                <a href={playlist.youtubePlaylistUrl} target="_blank" rel="noopener noreferrer"
                                    className="mt-5 w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-bold text-sm cursor-pointer"
                                    style={{
                                        background: 'linear-gradient(135deg, var(--color-primary), var(--color-primary-light))',
                                        color: '#fff', boxShadow: 'var(--shadow-warm)',
                                    }}>
                                    <PlayCircle size={16} /> Continue Watching
                                </a>
                            ) : (
                                <button onClick={() => enrollInCourse(playlist.slug, playlist.videoCount)}
                                    className="mt-5 w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-bold text-sm cursor-pointer"
                                    style={{
                                        background: 'linear-gradient(135deg, var(--color-primary), var(--color-primary-light))',
                                        color: '#fff', border: 'none', boxShadow: 'var(--shadow-warm)',
                                    }}>
                                    <PlayCircle size={16} /> Enroll — Free
                                </button>
                            )}
                        </div>

                        {relatedFinal.length > 0 && (
                            <div className="card p-5">
                                <h3 className="text-base font-bold mb-4" style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-primary)' }}>
                                    More Series
                                </h3>
                                <div className="flex flex-col gap-2">
                                    {relatedFinal.map((p) => <RelatedCard key={p.slug} playlist={p} />)}
                                </div>
                            </div>
                        )}
                    </div>
                </div>
            </div>
        </div>
    );
}
